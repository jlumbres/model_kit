...
def apply_zern_arb(raw_surf, dust_mask, nt, opt_parms, write_file=False):
    ...
    mask_vec = np.reshape(dust_mask, (vec1d))
            ...
            if write_file == True:
                ...
                data_diam = np.shape(mask)[0] * opt_parms['latres'] * u.pix
                ca = (data_diam/sp['value'][sp['label'].index('diam_100')].to(u.m))*100
                header['diam_ca'] = (data_diam.value, 'Data diameter at clear aperture [{0}]'.format(data_diam.unit))
                header['clear_ap'] = (ca.value, 'Clear aperture [percent]')

                surf_name = 'flat_{0}_n{1}_{2}CA_step{3}'.format(opt_parms['label'],nf+1,opt_parms['ca'],ns)
                fits.writeto(fits_folder+surf_name+'_bigdust_mask.fits', dust_set[nf][ns], header, overwrite=True)
                ...
                fits.writeto(fits_folder+surf_name+'_z{0}_surf.fits'.format(nt), final_surf, header, overwrite=True)

    return final_surf * raw_surf.unit
